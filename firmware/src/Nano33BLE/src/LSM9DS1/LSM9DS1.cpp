...
void LSM9DS1Class::readComplete(int event)
{
    ...
    // Start by requesting gyrometer data
    } else if (readstate == ST_IDLE) {
        address = LSM9DS1_ADDRESS;
        txbuffer[0] = 0x80 | LSM9DS1_OUT_X_G;
        txlen = 1;
        readstate = ST_REQGYR;

    // Then read the gyro data
    } else if(readstate == ST_REQGYR) {
        address = LSM9DS1_ADDRESS;
    ...
    // Request Accel Data
        ...
        address = LSM9DS1_ADDRESS;
        rxbuffer = (char*)ACCdata;
        rxlen = sizeof(ACCdata);
        txbuffer[0] = 0x80 | LSM9DS1_OUT_X_XL;
        readstate = ST_READACC;

    } else if (readstate == ST_READACC) {
        address = LSM9DS1_ADDRESS_M;
        rxbuffer = (char*)MAGdata;
        rxlen = sizeof(MAGdata);
        txbuffer[0] = 0x80 | LSM9DS1_OUT_X_L_M;
        readstate = ST_READMAG;
...
int LSM9DS1Class::getAllData(float (&acc)[3],float (&mag)[3],float (&gyr)[3])
{
    ...
    if(readstate == ST_IDLE) {
        watch.reset();
        readComplete();
        return -1; // Wasn't started
    } else if(readstate == ST_DONE) {
        watch.reset();
        ...
        return 1; // In the middle of a read
    }
}
